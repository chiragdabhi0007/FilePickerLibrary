...
import com.tops.filepicker.adapter.PhotoGridAdapter
import com.tops.filepicker.cursors.loadercallbacks.FileResultCallback
import com.tops.filepicker.model.Media
import com.tops.filepicker.model.PhotoDirectory
import com.tops.filepicker.utills.AndroidLifecycleUtils
import com.tops.filepicker.utills.ImageCaptureManager
import com.tops.filepicker.utills.ImageUtills
import com.tops.filepicker.utills.MediaStoreHelper
...
class MediaDetailPickerFragment : BaseFragment(), FileAdapterListener {
        ...
        view?.let { _ ->
            ...
            context?.let {
                if (photoGridAdapter != null) {
                    ...
                    photoGridAdapter = PhotoGridAdapter(it, mGlideRequestManager, medias, PickerManager.selectedPhotos, PickerManager.isEnableCamera, this)
                    ...
                    photoGridAdapter?.setCameraListener(View.OnClickListener {
                        if ((fileType == FilePickerConst.MEDIA_TYPE_IMAGE)) {
                            ...
                            try {
                                ...
                                startActivityForResult(intent, ImageCaptureManager.REQUEST_VIDEO_CAPTURE)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            ImageCaptureManager.REQUEST_VIDEO_CAPTURE -> if (resultCode == Activity.RESULT_OK) {

                context?.let {
                    if (PickerManager.getMaxCount() == 1) {


                        PickerManager.add(ImageUtills.getFilePath(it, data!!.data), FilePickerConst.FILE_TYPE_MEDIA)
                        mListener?.onItemSelected()
                    } else {
                        Handler().postDelayed({ getDataFromMedia() }, 1000)
                    }
